import type { WorldState, SelectedObject, DecorationKind } from '../core/sceneTypes'
import type { IndoorSprites } from './indoorSprites'
import { TILE_SIZE, SCENE_PAD_L, SCENE_PAD_R, SCENE_PAD_T, SCENE_PAD_B, tileCenter } from '../core/characters'
import { CHAR_FRAME_W, CHAR_FRAME_H, DIR_ROW, WALK_FRAMES, IDLE_FRAME } from './indoorSprites'
import { COLS, ROWS, DECO_TILE_SIZE, BOARD_COL, DASH_COL, EXIT_COL, PORTAL_COL } from './indoorWorldState'

// Logical canvas dimensions (must match SceneCanvas / OfficeScene)
...
// Character sprite drawn at 4x pixel scale -> 64x128 CSS px (matches 4x tile scale)
const CHAR_SCALE = 4
...
export function renderIndoor(
  ctx: CanvasRenderingContext2D,
  canvas: HTMLCanvasElement,
  world: WorldState,
  sprites: IndoorSprites | null,
  ...
  if (sprites) {
    ...
    // Bulletin board: 5x3, starts at BOARD_COL
    ctx.drawImage(
    ...
    // Dashboard: 5x3, starts at DASH_COL (3-tile gap after board)
    ctx.drawImage(
    ...
    // Exit door: 2x2, wall-mounted at EXIT_COL, hangs 1 tile into wall
    ctx.drawImage(
      sprites.exitDoor,
    ...
    // Portal: 2x2, wall-mounted at PORTAL_COL, hangs 1 tile into wall
    ctx.drawImage(
      sprites.portal,
  ...
  for (const ch of world.characters.values()) {
    ...
    if (sprites) {
      const spriteSheet = ch.id === 'astraea' ? sprites.astraea : sprites.gaia
      ...
      if (ch.dir === 'LEFT') {
        ...
        ctx.translate(cx, 0)
        ctx.scale(-1, 1)
        ctx.drawImage(spriteSheet.img, sx, sy, CHAR_FRAME_W, CHAR_FRAME_H,
