import type { WorldState, GridTile, Decoration, DecorationKind } from '../core/sceneTypes'
...
// Shared between renderer (drawing) and collision (pathfinding blocked set).
...
// 22 x 64 px = 1408 px wide
// 12 x 64 px =  768 px tall
export const COLS = 22
export const ROWS = 12

// Target tile coords
// Layout: 1-tile | Board (5-tile) | 3-tile | Dashboard (5-tile)
// Bulletin board: 5x3, visual cols 1-5, hangs 1 tile into wall
...
// Dashboard: 5x3, visual cols 8-12, 2-tile gap after board
...
// Exit door: 2x2, wall-mounted at cols 17-18, hangs 1 tile into wall
export const EXIT_COL      = 17
export const EXIT_WALK_COL = 17   // in front of exit door
export const EXIT_WALK_ROW = 2

// Portal: 2x2, wall-mounted at cols 20-21, hangs 1 tile into wall
...
const DECORATIONS: Decoration[] = [
  ...
  // shelf1 (4x2) -- left wall, row 4 (moved down 1 tile)
  { col: 0,  row: 4, kind: 'shelf1' },
  // pc desks (2x1.5) -- side by side, row 4
  { col: 7,  row: 4, kind: 'pcDesk' },
  { col: 9, row: 4, kind: 'pcDesk' },
  // 4-tile walkway gap: rows 5-8 (no furniture)
  // shelf3 (4x2) -- left wall, row 8
  { col: 0,  row: 8, kind: 'shelf3' },
  // shelf2 (4x2) -- move to the right side of shelf3 (3-tile gap), for SKILL shelf
  { col: 7,  row: 8, kind: 'shelf2' },

  // -- VERTICAL DIVIDER: pot column at col 14 --
  // pots are 1x2 tiles tall -> one per 2 rows = continuous green wall
  ...
  // lounge: black sofa top (2x2), pink sofas x2 each side, table bottom (2x2)
...
export function createIndoorWorldState(): WorldState {
  ...
  // Bulletin board (5x3) -> grid rows 0-1, cols 1-5
  // Dashboard (5x3)     -> grid rows 0-1, cols 9-13
  // Exit door (2x2)     -> grid row 0, cols 17-18
  // Portal (2x2)        -> grid row 0, cols 20-21
